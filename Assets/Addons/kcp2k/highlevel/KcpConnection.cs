    ...
    public abstract class KcpConnection
    {
        ...
        void HandleTimeout(uint time)
        {
            ...
            if (time >= lastReceiveTime + timeout)
            {
                ...
                OnError?.Invoke(ErrorCode.Timeout, $"KCP: Connection timed out after not receiving any message for {timeout}ms. Disconnecting.");
                Disconnect(KcpDisconnectCode.Timeout);
            }
        }
        ...
        void HandleDeadLink()
        {
            ...
            {
                // pass error to user callback. no need to log it manually.
                OnError?.Invoke(ErrorCode.Timeout, $"KCP Connection dead_link detected: a message was retransmitted {kcp.dead_link} times without ack. Disconnecting.");
                Disconnect(KcpDisconnectCode.Timeout);
            }
        }
        ...
        void HandleChoked()
        {
            ...
            {
                // pass error to user callback. no need to log it manually.
                OnError?.Invoke(ErrorCode.Congestion,
            ...
            if (msgSize > 0)
            {
                ...
                if (msgSize <= kcpMessageBuffer.Length)
                {
                    ...
                    else
                    {
                        ...
                        // pass error to user callback. no need to log it manually.
                        OnError?.Invoke(ErrorCode.InvalidReceive, $"Receive failed with error={received}. closing connection.");
                        Disconnect();
                    }
                }
                ...
                {
                    // pass error to user callback. no need to log it manually.
                    OnError?.Invoke(ErrorCode.InvalidReceive, $"KCP: possible allocation attack for msgSize {msgSize} > buffer {kcpMessageBuffer.Length}. Disconnecting the connection.");
                    Disconnect();
                }
            }
            ...
            if (ReceiveNextReliable(out KcpHeader header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    ...
                    case KcpHeader.Disconnect:
                    {
                        ...
                        // pass error to user callback. no need to log it manually.
                        OnError?.Invoke(ErrorCode.InvalidReceive, $"KCP: received invalid header {header} while Connected. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                }
            ...
            while (ReceiveNextReliable(out KcpHeader header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    ...
                    case KcpHeader.Data:
                    {
                        ...
                        {
                            // pass error to user callback. no need to log it manually.
                            OnError?.Invoke(ErrorCode.InvalidReceive, "KCP: received empty Data message while Authenticated. Disconnecting the connection.");
                            Disconnect();
                        }
                        break;
                    }
                    ...
                    case KcpHeader.Disconnect:
                    {
                        ...
                        Disconnect(GetDisconnectCode(message));
                        break;
                    }
                }
            }
        ...
        private byte GetDisconnectCode(ArraySegment<byte> segment)
        {
            return segment.Count > 0 ? segment[0] : KcpDisconnectCode.Default;
        }
        ...
        public void TickIncoming()
        {
            ...
            catch (SocketException exception)
            {
                ...
                // pass error to user callback. no need to log it manually.
                OnError?.Invoke(ErrorCode.ConnectionClosed, $"KCP Connection: Disconnecting because {exception}");
                Disconnect();
            }
            ...
            catch (Exception exception)
            {
                // unexpected
                // pass error to user callback. no need to log it manually.
                OnError?.Invoke(ErrorCode.Unexpected, $"KCP Connection: unexpected Exception: {exception}");
                Disconnect();
            }
        }
                ...
                switch (channel)
                {
                    ...
                    case (byte)KcpChannel.Unreliable:
                    {
                        ...
                        else
                        {
                            // should never happen
                            // pass error to user callback. no need to log it manually.
                            OnError?.Invoke(ErrorCode.InvalidReceive, $"KCP: received unreliable message in state {state}. Disconnecting the connection.");
                            Disconnect();
                        }
                        break;
                    }
                    default:
                    {
                        ...
                        // pass error to user callback. no need to log it manually.
                        OnError?.Invoke(ErrorCode.InvalidReceive, $"Disconnecting connection because of invalid channel header: {channel}");
                        Disconnect();
                        break;
                    }
                }
        ...
        public void SendData(ArraySegment<byte> data, KcpChannel channel)
        {
            ...
            {
                // pass error to user callback. no need to log it manually.
                OnError?.Invoke(ErrorCode.InvalidSend, "KcpConnection: tried sending empty message. This should never happen. Disconnecting.");
                Disconnect();
                return;
            }
        ...
        void SendDisconnect(byte[] data) => SendReliable(KcpHeader.Disconnect, data);

        protected virtual void Dispose() {}

        // disconnect this connection
        public void Disconnect(byte code = KcpDisconnectCode.Default)
        {

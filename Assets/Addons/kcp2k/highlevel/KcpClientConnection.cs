...
namespace kcp2k
{
    public class KcpClientConnection : KcpConnection
    {
        ...
        private void OnConnectFailed(string host)
        {
            // pass error to user callback. no need to log it manually.
            OnError(ErrorCode.DnsResolve, $"Failed to resolve host: {host}");
            OnDisconnected(KcpDisconnectCode.HostUnreachable);
        }
        ...
        public void RawReceive()
        {
            ...
            catch (SocketException ex)
            {
                ...
                Log.Info($"KCP ClientConnection: looks like the other end has closed the connection: {ex}");
                Disconnect();
            }
        }

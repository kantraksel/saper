...
namespace Mirror
{
    ...
    public class LocalConnectionToClient : NetworkConnectionToClient
    {
        ...
        private bool disconnectCallOnce;
        /// <summary>Disconnects this connection.</summary>
        public override void Disconnect(byte code)
        {
            if (disconnectCallOnce) return;
            disconnectCallOnce = true;

            DisconnectInternal();
            connectionToServer.DisconnectInternal();
            if (NetworkClient.connectState != ConnectState.Disconnecting)
                code = DisconnectCode.InternalError;
            NetworkClient.OnTransportDisconnected(code);

            disconnectCallOnce = false;
        }
    }
}
